//Modified by Jorge Torres, so that only Veff tree is written out.
...
//-------------------------------------------------- 
  ...
  TTree *AraTree3=new TTree("AraTree3","AraTree3"); //JCH: for effective volume
  double Veff_test,error_plus,error_minus;
  ...
  AraTree3->Branch("VeffOmega",&Veff_test);
  AraTree3->Branch("VeffOmega_errorplus",&error_plus);
  AraTree3->Branch("VeffOmega_errorminus",&error_minus);
...
if (settings1->INTERACTION_MODE==1)
{
	...
	error_plus*=1.0e-9; //units: km3str
	error_minus*=1.0e-9;
	AraTree3->Fill();
	AraTree3->Write();
}
